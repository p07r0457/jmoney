...
/**
 * This dialog box class prompts the user for a foreign currency amount
 ...
 * enter the foreign currency amount in the normal way in the credit or
 * debit columns.  However, in a simple transaction the two entries are
 * combined and displayed on one row.  This necessitates the creation
 * on another column to hold the amount of the income or expense.  However,
 * that column is so rarely used that it is not displayed by default.
 * As the column is not displayed by default, we cannot require the user
 * to enter data in it.
 * 
...
public class ForeignCurrencyDialog {
    ...
    public ForeignCurrencyDialog(Shell parent, EntryData item) {
        ...
        shell = new Shell(parent, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL | SWT.OK | SWT.CANCEL);
        shell.setText("Amount to Enter in Income and Expense Account");

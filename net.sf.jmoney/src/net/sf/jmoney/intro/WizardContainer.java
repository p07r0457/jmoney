 ...
 * page appears in the middle; and at the bottom of the page is message line and
 * a button bar containing Help, Next, Back, Close, and Standby/Maximize buttons
 * (or some subset).
 * </p>
 */
public class WizardContainer extends Composite implements IWizardContainer2 {
	...
	/**
	 * Indicates whether the container is in standby or full screen mode.
	 */
	 ...
	 */
	public WizardContainer(Composite parent, IWizard newWizard,
			IIntroPart introPart) {
	...
	public boolean close() {
		...
		// Close the container
		introPart.getIntroSite().getWorkbenchWindow().getWorkbench()
				.getIntroManager().closeIntro(introPart);
	...
	protected void createButtonsForButtonBar(Composite parent) {
		if (wizard.isHelpAvailable()) {
			helpButton = createButton(parent, IDialogConstants.HELP_LABEL,
					false);
		...
		closeButton = createButton(parent, IDialogConstants.CLOSE_LABEL, true);
		...
		standbyButton = createButton(parent, Messages.WizardContainer_Standby,
				false);
	...
	protected Control createButtonBar(Composite parent) {
		...
		layout.makeColumnsEqualWidth = false; // necessary so back and next
												// buttons touch
	 ...
	 * wizard to add extra pages, then <code>super.createContents</code> to
	 * create the controls. It then calls
	...
	protected Composite createDialogArea(Composite parent) {
		...
		// These margins are set to zero if the user puts the container in
		// standby mode.
	...
	private Composite createPreviousAndNextButtons(Composite parent) {
		...
		backButton = createButton(composite, IDialogConstants.BACK_LABEL, false);
		...
		nextButton = createButton(composite, IDialogConstants.NEXT_LABEL, false);
	 ...
	 * @return MessageDalog
	 * 
	 *         private MessageDialog createWizardClosingDialog() { MessageDialog
	 *         result = new MessageDialog(getShell(), JFaceResources
	 *         .getString("WizardClosingDialog.title"), //$NON-NLS-1$ null,
	 *         JFaceResources.getString("WizardClosingDialog.message"),
	 *         //$NON-NLS-1$ MessageDialog.QUESTION, new
	 *         String[]{IDialogConstants.OK_LABEL}, 0); return result; }
	 */
	...
	protected void standbyPressed() {
		...
		introPart.getIntroSite().getWorkbenchWindow().getWorkbench()
				.getIntroManager().setIntroStandby(introPart, !standby);
	}

	/**
	 * Called externally to notify this object that the standby mode has
	 * changed.
	 * 
	 * The text in the standby/maximize button is switched as appropriate.
	 * 
	 * @param standby
	 */
	public void standbyStateChanged(boolean standby) {
		...
		standbyButton.setText(standby ? Messages.WizardContainer_Maximize
				: Messages.WizardContainer_Standby);
	 ...
	 */
	/*
	 * protected IProgressMonitor getProgressMonitor() { return
	 * progressMonitorPart; }
	 */
	/**
	 ...
	 * @param key
	 *            the key
	 * @see #saveEnableStateAndSet private void restoreEnableState(Control w,
	 *      Map h, String key) { if (w != null) { Boolean b = (Boolean)
	 *      h.get(key); if (b != null) w.setEnabled(b.booleanValue()); } }
	 */
	 ...
	 * @see #saveUIState private void restoreUIState(Map state) {
	 ...
	 *      restoreEnableState(helpButton, state, "help"); //$NON-NLS-1$ Object
	 *      pageValue = state.get("page"); //$NON-NLS-1$ if(pageValue !=
	 *      null)//page may never have been created ((ControlEnableState)
	 *      pageValue).restore(); }
	 */
	...
	public void run(boolean fork, boolean cancelable,
		...
		throw new RuntimeException(Messages.WizardContainer_NotImplemented);
	}
	 ...
	 * @see #restoreEnableState(Control, Map, String) private void
	 *      saveEnableStateAndSet(Control w, Map h, String key, boolean enabled)
	 *      { if (w != null) { h.put(key, new Boolean(w.getEnabled()));
	 ...
	 * @see #restoreUIState private Map saveUIState(boolean keepCancelEnabled) {
	 *      Map savedState = new HashMap(10); saveEnableStateAndSet(backButton,
	 *      savedState, "back", false); //$NON-NLS-1$
	 *      saveEnableStateAndSet(nextButton, savedState, "next", false);
	 *      //$NON-NLS-1$ saveEnableStateAndSet(closeButton, savedState,
	 *      "close", false); //$NON-NLS-1$ saveEnableStateAndSet(standbyButton,
	 *      savedState, "standby", false); //$NON-NLS-1$
	 *      saveEnableStateAndSet(helpButton, savedState, "help", false);
	 *      //$NON-NLS-1$ if(currentPage != null) savedState.put("page",
	 *      ControlEnableState.disable(currentPage.getControl())); //$NON-NLS-1$
	 *      return savedState; }
	 */
	/**
	 ...
	 *            the cursor
	 * 
	 *            private void setDisplayCursor(Cursor c) { Shell[] shells =
	 *            getShell().getDisplay().getShells(); for (int i = 0; i <
	 *            shells.length; i++) shells[i].setCursor(c); }
	 */
	...
	public void showPage(IWizardPage page) {
		...
		/*
		 * // Update for the new page ina busy cursor if possible // getContents
		 * returns the top level control for the window, // the shell? if
		 * (getContents() == null) updateForPage(page); else {
		 */
		{
	 ...
	 *            the wizard
	 * 
	 *            private void updateSizeForWizard(IWizard sizingWizard) { Point
	 *            delta = new Point(0, 0); IWizardPage[] pages =
	 *            sizingWizard.getPages(); for (int i = 0; i < pages.length;
	 *            i++) { // ensure the page container is large enough Point
	 *            pageDelta = calculatePageSizeDelta(pages[i]); delta.x =
	 *            Math.max(delta.x, pageDelta.x); delta.y = Math.max(delta.y,
	 *            pageDelta.y); } if (delta.x > 0 || delta.y > 0) { // increase
	 *            the size of the shell Shell shell = getShell(); Point
	 *            shellSize = shell.getSize(); this.setSize(shellSize.x +
	 *            delta.x, shellSize.y + delta.y); } }
	 */

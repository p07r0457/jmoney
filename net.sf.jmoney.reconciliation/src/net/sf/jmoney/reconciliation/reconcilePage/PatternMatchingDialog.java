...
public class PatternMatchingDialog extends Dialog {
	 ...
	 */
	public PatternMatchingDialog(Shell parentShell, PatternMatcherAccount account, Collection<EntryData> sampleEntries) {
		super(parentShell);
				...
				for (MemoPattern pattern: sortedPatterns) {
					...
					if (m.matches()) {
						...
						if (propertyAccessor == MemoPatternInfo.getMemoAccessor()
							...
							String format = (String)pattern.getPropertyValue(propertyAccessor);
							if (format == null) {
								return "";
							} else {
							...
							return pattern.getAccount().getName();
